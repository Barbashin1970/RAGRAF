...
function SearchDemo() {
  ...
  // Дефолт: ВСЕ регламенты ВКЛЮЧЕНЫ (empty set = «никто не отключён»).
  // Новый пользователь после деплоя сразу видит «N / N включено» и получает
  // ответ с упоминанием конкретных доменов из корпуса. Раньше пробовали
  // opt-in (все выкл) ради скорости первого «Привет» на cold-start Ollama,
  // но это сбивало с толку — ответ выглядел оторванным от данных, а на
  // cloud-провайдере (Cerebras) скорость и так не проблема.
  ...
  // Раньше тут был useEffect, который при первой загрузке корпуса принудительно
  // выключал все регламенты. Удалено — теперь дефолт = все включены (см. инициализацию
  // disabledRegulationIds выше). При желании пользователь снимает галки вручную
  // или применяет пресет «Резюме документа», который и так делает disable_all.
        ...
        >
          {turns.length === 0 && !chat.isPending && (
            <EmptyChatHint
              ...
              // Раньше тут стоял forceSkipRegulations: true — это был трюк
              // ради скорости первого ответа на Ollama. Но новичок при этом
              // получал общий ответ без упоминания конкретных регламентов, и
              // не понимал зачем нужна левая панель. Теперь warmup идёт с
              // retrieval'ом — LLM перечисляет реальные домены из контекста.
              onWarmup={() => send(WARMUP_QUERY)}
...
// Стартовый «Привет»: задача — чтобы новичок СРАЗУ увидел, что у системы
// в контексте лежат конкретные регламенты, а не абстрактный ИИ-ассистент.
// Поэтому retrieval НЕ выключаем (раньше выключали ради скорости на cold-start
// Ollama, но это сбивало с толку — ответ выглядел оторванным от данных).
// LLM явно просим:
//   1) перечислить домены, которые сейчас подключены слева;
//   2) пригласить кликнуть на чекбокс домена / отдельного регламента;
//   3) предложить 1-2 конкретных вопроса из ассортимента примеров.
const WARMUP_QUERY =
  'Привет! Это первый запуск Студии. Сделай короткое (4-5 пунктов) знакомство: ' +
  '1) представься как ИИ-помощник аналитика RAGRAF; ' +
  '2) перечисли через запятую КОНКРЕТНЫЕ домены/регламенты, которые у тебя ' +
  'сейчас в контексте слева (бери их из retrieved-источников); ' +
  '3) подскажи, что в левой панели можно включать/выключать регламенты по галочкам ' +
  'или целыми доменами — это сужает контекст ответа; ' +
  '4) предложи 2 примера полезных вопросов по этим конкретным регламентам ' +
  '(не общих, а с упоминанием параметров или сценариев); ' +
  '5) укажи, что в правой панели можно выбрать модель и пресет сценария ' +
  '(«Резюме документа», «Извлечь параметры», «Сравнить регламенты», «Краткий ответ»).'
  ...
  return (
    <div className="flex h-full flex-col items-center justify-center py-8 text-center">
      ...
      {/* Главная стартовая кнопка — крупная, в виолетовом стиле Author Layer.
          На Ollama первый запрос грузит qwen2.5:7b в RAM (~4.4 ГБ, 30-60 сек
          на M2 Air); дальше всё быстро. Регламенты при этом ВКЛЮЧЕНЫ — LLM
          перечислит реальные домены из контекста, чтобы новичок понимал что
          в левой панели лежат конкретные данные, а не пустой контекст. */}
      <button
        onClick={onWarmup}
        ...
        title="Отправляет приветствие — LLM расскажет, какие регламенты сейчас подключены слева и предложит примеры вопросов."
      >
        <Sparkles size={14} />
