...
flow-снапшоты и audit-журнал переживали редеплой:

- **Volume должен быть смонтирован на `/data`** перед первым деплоем — иначе
  файл пишется в RW-слой контейнера и пропадает.
- **Локальный `backend/data/regulations.duckdb` исключён из Docker image**
  через `.dockerignore`. Раньше он шёл в `_seed_data/`: при ротации
  «повреждённого» файла probe'ом в `start.sh` сид-блок копировал
  dev-snapshot поверх прод-Volume, и пользователь терял twins. Сейчас в
  image только `fixtures/` + `demo_documents/`.
- **Probe в `start.sh` НЕ делает write-roundtrip** — только `SELECT 1` +
  `CHECKPOINT`. Раньше UPDATE/COMMIT-цикл на `parameters` давал
  false-positive ротацию при минимальном расхождении версии DuckDB между
  dev и prod → файл уходил в `.broken-*` → seed возвращал старое состояние.
- **Defense-in-depth в seed-цикле**: `find ... ! -name 'regulations.duckdb'
  ! -name 'flows' ! -name 'versions' …` — даже если кто-то снова сломает
  `.dockerignore`, сид-блок сам откажется копировать runtime-артефакты.

**Если twins всё-таки потерялись** (старая версия + ротация ранее):
проверь `/data/regulations.duckdb.broken-*` — оригинальная БД лежит там
до следующего ручного удаления Volume. Бэкап восстанавливается через
`mv .broken-<ts> regulations.duckdb`.
