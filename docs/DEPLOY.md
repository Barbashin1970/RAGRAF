...
2. После первого деплоя Railway смонтирует Volume; `start.sh` обнаружит
   ...
   `/srv/_seed_data` (фикстуры + demo-документы).
3. **Никакая БД из dev-машины не попадает в image** (исключено через
   `.dockerignore`). На первом старте `init_db()` создаёт `regulations.duckdb`
   с нуля из фикстур; пользовательские данные (twins/flows/versions/audit)
   накапливаются на Volume и переживают редеплои.

### 3.2.1 Persistence-контракт (важно — почему ранее терялись twins)

Чтобы цифровые двойники (`processes`), пользовательские правки регламентов,
flow-снапшоты и audit-журнал переживали редеплой, реализована
**4-уровневая защита**:

**Уровень 1: dev-DB не попадает в image.** `backend/data/regulations.duckdb`
(и `flows/`, `versions/`, `ragu_store/`, `source_documents/`) исключены
через `.dockerignore`. Раньше dev'овский файл копировался в `_seed_data/`:
при ротации «повреждённого» прод-файла сид-блок копировал dev-snapshot
поверх Volume — пользователь терял twins. Сейчас в image **только**
`fixtures/` + `demo_documents/`.

**Уровень 2: read-only probe.** `start.sh` делает только `SELECT 1` +
`CHECKPOINT`. Раньше выполнялся `UPDATE … COMMIT` на `parameters` —
давал false-positive ротации при минимальном расхождении версии DuckDB
между dev и prod (видно по логам: 13+ `.broken-*` файлов за сутки).

**Уровень 3: WAL-only recovery.** Если probe всё-таки падает (например,
WAL-replay фейлит после SIGKILL'а на редеплой — типичная ошибка
«Failure while replaying WAL file: GetDefaultDatabase»), `start.sh` сначала
ротирует **только** `regulations.duckdb.wal` и пробует открыть БД ещё раз.
В 90% случаев главный файл валиден — пользователь теряет только
незакоммиченные транзакции (последние секунды до SIGKILL'а), все ранее
сохранённые twins/правки/audit остаются. Только если и без WAL открыть
не удаётся — ротируется главный файл (последнее средство).

**Уровень 4: CHECKPOINT на shutdown.** FastAPI lifespan handler на yield/exit
форсит `CHECKPOINT` — флашит WAL в главный файл перед остановкой контейнера.
Это резко снижает шанс WAL-corruption на следующем старте: контейнер
останавливается с пустым WAL → новый старт открывает без replay-фазы.

**Defense-in-depth в seed-цикле**: `find ... ! -name 'regulations.duckdb'
! -name 'flows' ! -name 'versions' …` — даже при регрессии `.dockerignore`
сид сам откажется копировать runtime-артефакты.

**Если twins всё-таки потерялись** (старые `.broken-*` от прежних версий):
проверь `/data/regulations.duckdb.broken-*` — оригинальная БД лежит там
до следующего ручного удаления Volume. Восстановление:
```bash
# зайти в Railway shell контейнера
mv /data/regulations.duckdb.broken-<ts> /data/regulations.duckdb
