...
> Среда проектирования **операционного цифрового двойника управленческих
> процессов** на базе фреймворка Σ Сигма. Аналитик-методолог собирает
> регламенты, связывает их с датчиками через триггеры, объединяет в
> процесс на странице `/twins` и экспортирует артефакт (Turtle /
> SIGMA-bundle ZIP) в исполнительный движок CORE Σ.
>
> **Цель проекта:** не «редактор регламентов вообще», а **digital twin
> of governance** — нормативно обоснованная модель того, как организация
> реагирует на цепочку событий. Это целевая фича Σ, к которой движется
> весь стек. Подробное описание модели и сравнение с альтернативами:
> [`RULES-MANAGEMENT.md`](RULES-MANAGEMENT.md) §11.
>
...
### 1.3 Целевая фича — Process (цифровой двойник)

`Process` — first-class сущность, объединяющая 2–N регламентов в **именованный
цифровой двойник управленческого процесса**. Реализуется страницей `/twins`,
DuckDB-таблицей `processes` и REST `/api/processes/*`. К двойнику привязаны
все остальные компоненты:

- регламенты — нормативное ядро;
- триггеры (`RegulationTrigger`) — декларация «вход регламента → датчик/событие»;
- библиотека датчиков — JSON-контракты событий, на которые подписаны триггеры;
- композиция (`source_regulation`/`source_output`) — output одного регламента
  служит триггером для другого, замыкая цепочку реагирования;
- симулятор Flow Editor — прогон одного регламента; симулятор цепочки
  целиком — следующая фаза;
- экспорт SIGMA-bundle (ZIP) / объединённого Turtle — артефакт для исполнительного
  движка CORE Σ или нормативного аудита, **не зависит от RAGRAF** (читается
  стандартными OWL-инструментами: Apache Jena, Protégé).

Это и есть **operational digital twin of governance** — отличается от
n8n-семейства (workflow automation), от Camunda+DMN (процесс как
последовательность задач, без онтологии) и от digital twin физического
актива (Siemens MindSphere / AVEVA — привязка к одному оборудованию).
Подробный разбор: [`RULES-MANAGEMENT.md`](RULES-MANAGEMENT.md) §11.

### 1.4 Что система НЕ делает
