...
> **Локальная студия аналитика. Спутник станции Σ Сигма.**
>
> RAGRAF — **самодостаточный продукт**, который ставится на ноутбук аналитика
> и работает полностью offline. Не требует подключения к ядру СИГМЫ, серверу
> заказчика или интернету (после установки). В архитектуре «Σ Сигма» он
> играет роль **спутника**: где центральная станция СИГМА исполняет регламенты
> в проде и связана с инфраструктурой заказчика — спутник RAGRAF их **проектирует**
> и **шлифует** до того, как они улетят в эту станцию.

```
                 ┌──────────────────────────────────┐
                 │  📡 СТАНЦИЯ Σ (на стороне        │
                 │     заказчика — муниципалитет,   │
                 │     ЦСИИ, отраслевая ИС)         │
                 │  • датчики, ETL, события         │
                 │  • runtime: исполнение регла-    │
                 │    ментов, дашборд оператора     │
                 │  • дашборд руководителя          │
                 └──────────────┬───────────────────┘
                                ▲
                  SIGMA-bundle  │  ZIP с артефактами
                  (data.ttl +   │  ↑↓ round-trip
                   shapes.ttl + │
                   regulation.json + flow.json + source)
                                │
                                ▼
   ┌─────────────────────────────────────────────────┐
   │ 🛰 СПУТНИК RAGRAF (локально на ноутбуке         │
   │    аналитика; macOS / Windows installer)        │
   │                                                  │
   │  • импорт PDF/DOCX → извлечение через RAGU/LLM  │
   │  • редактор регламентов (Form / Flow / SHACL)   │
   │  • библиотека датчиков + паспорта модулей       │
   │  • цифровой двойник процесса (composition)      │
   │  • симулятор + аудит-журнал                     │
   │  • экспорт SIGMA-bundle ZIP                     │
   │                                                  │
   │  ВСЕ ДАННЫЕ ЛОКАЛЬНО (DuckDB в ~/RAGRAF/data/). │
   │  Не слушает события датчиков, не исполняет в    │
   │  проде — это работа СТАНЦИИ.                    │
   └─────────────────────────────────────────────────┘
```

**Почему две системы, а не одна:**
- Аналитик работает с *черновиками* — перебирает варианты, тестирует на симуляторе.
  Это другой режим работы, чем production-runtime станции.
- Локальная установка снимает требование «дать аналитику доступ к проду» —
  безопасность, compliance, регламенты доступа к ПДн.
- RAGRAF тиражируется на ноутбук каждого аналитика; СИГМА централизована
  в инфраструктуре заказчика.
- RAGRAF может работать **в отрыве от СИГМЫ** — для команд, у которых её нет:
  свой regulation engineering, экспорт в любой downstream-формат.

**Связь со станцией:**
- Готовые регламенты экспортируются одним ZIP (data.ttl + shapes.ttl +
  regulation.json + flow.json + source-документ-основание).
- Round-trip — импорт обратно если СИГМА что-то меняет.
- НЕ запускает регламенты в проде, НЕ слушает датчики — это контур станции.

---

**Что внутри спутника** (детали ниже): аналитик описывает регламенты как графы
реакций на события от датчиков и других регламентов; платформа валидирует их
по нормативным основаниям (SHACL/OWL), исполняет на симуляторе и экспортирует
артефакт в исполнительный движок СИГМЫ.
