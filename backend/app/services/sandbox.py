...
"""
    ...
    if not hits:
        ...
        if doc_chunks:
            ...
            # Conversational fallback: контекста нет совсем (нет ни регламентов
            # в retrieval'е, ни документов аналитика). Раньше тут было «отвечать
            # не по чему — предложи включить» — LLM писала отказ даже на простой
            # вопрос вроде «Привет, что ты умеешь?». Теперь даём ей чёткое описание
            # возможностей RAGRAF чтобы welcome-сценарий работал.
            base_rules = (
                "Ты — ИИ-ассистент аналитика в системе RAGRAF. Отвечай естественно "
                "и дружелюбно по сути вопроса.\n\n"
                "Твои возможности:\n"
                "• Q&A по корпусу регламентов: тепловые сети, ЖКХ, безопасность "
                "кампуса, экология. Семантический поиск через bge-m3.\n"
                "• Суммаризация документов аналитика (PDF/DOCX) — загружаются "
                "слева во вкладке «Документы».\n"
                "• Извлечение параметров с допусками из произвольного текста.\n"
                "• Сравнение регламентов: где значения совпадают/расходятся.\n"
                "• Пресеты сценариев в правой панели меняют стиль ответа.\n\n"
                "Если для конкретного ответа нужен контекст — предложи пользователю "
                "включить регламенты в левой панели или загрузить документ."
            )
