...
def seed_if_empty() -> None:
    """Засеять стартовый набор модулей.

    Per-id идемпотентность: для каждого seed-модуля вставляем только
    если такого id ещё нет. Раньше использовался COUNT(*) → if any rows,
    skip — это ломало восстановление, когда другая миграция уже добавила
    свои модули раньше: seed_if_empty видел не-пустую таблицу и пропускал
    9 базовых модулей. После этого изменения порядок выполнения сидов
    и миграций не имеет значения — все базовые модули гарантированно
    появляются, пользовательские правки конкретных id не затрагиваются.
